...
def apply_sidebar_filters(df: pd.DataFrame) -> pd.DataFrame:
    """Sidebar-ийн бүх шүүлтүүрийг нэг газар хэрэгжүүлнэ.

    Ингэснээр KPI, chart, table, customer search, export бүгд ижил
    filtered dataset ашиглана. Multiselect хоосон бол үр дүн хоосон болно.
    """
    out = df.copy()

    if COL_STATUS1 in out.columns:
        out = out[out[COL_STATUS1].isin(s1_sel)].copy()
    if "gender_label" in out.columns:
        out = out[out["gender_label"].isin(g_sel)].copy()
    if age_r and "age" in out.columns:
        out = out[pd.to_numeric(out["age"], errors="coerce").between(*age_r)].copy()
    if "marital_label" in out.columns:
        out = out[out["marital_label"].isin(m_sel)].copy()
    if "edu_name" in out.columns:
        out = out[out["edu_name"].isin(e_sel)].copy()
    if lt_sel != "Бүгд" and "location_type" in out.columns:
        out = out[out["location_type"] == lt_sel].copy()
    if od_r and COL_MAX_AOD in out.columns:
        out = out[out[COL_MAX_AOD].between(*od_r)].copy()

    return out

# Данс түвшний filtered дата — бүх loan-level хэсгүүд үүнийг ашиглана.
df_acct = apply_sidebar_filters(df_raw)

# Харилцагч түвшний дата — filtered loan dataset дээрээс дахин нэгтгэнэ.
# Ингэснээр харилцагчийн KPI/chart/table/export мөн sidebar filter-ийг дагана.
df_cust_full = build_customer_df(df_raw) if has_cust else pd.DataFrame()
df_cust = build_customer_df(df_acct) if has_cust else pd.DataFrame()
...
with TAB_LOAN:
    ...
    # oa_d = df_acct.loc[df_acct[COL_STATUS1]=="O_active", COL_MAX_AOD] if COL_STATUS1 in df_acct.columns and COL_MAX_AOD in df_acct.columns else pd.Series()
    ...
    _oa_cust_od = df_acct[df_acct[COL_STATUS1]=="O_active"].groupby(COL_CUST)[COL_MAX_AOD].max() if COL_STATUS1 in df_acct.columns and COL_CUST in df_acct.columns and COL_MAX_AOD in df_acct.columns else pd.Series()
    ...
    with L3:
        if not has_cust:
            ...
            def build_cust_options(d):
                """Filtered датагаас зээлийн тоо + MAX хэтрэлт + төлөв харуулсан dropdown үүсгэнэ."""
                if COL_CUST not in d.columns or d.empty:
                    return [], {}
            ...
            labels, code_map = build_cust_options(df_acct)
            ...
            if not labels:
                ...
                rows_ = df_acct[df_acct[COL_CUST].astype(str).str.strip() == selected_code]
    ...
    with C9:
        ...
        for col, lbl in NUM.items():
            if col not in df_cust.columns:
                continue
            tmp = df_cust[[col, OD]].dropna()
            if len(tmp) > 5:
                rows_c.append({
                    "Хувьсагч": lbl,
                    "Корреляц": round(tmp[col].corr(tmp[OD]), 4),
                    "n": len(tmp),
                })
